...
{	
	...
	if (numLights > 0 ) {
		...
		if (dist < 0.0) tempColor = matDiff*lightCol0*cosTheta+matSpec*lightCol0;
		else tempColor = matDiff*lightCol0*cosTheta/(dist*dist)+matSpec*lightCol0*pow(cosAlpha,5)/(dist*dist);
	}
